...
static int callback(void *NotUsed, int ncolumns, char **fields, char **columns) {
	for(int i=0; i<ncolumns; i++) std::cerr<<columns[i]<<" = "<<(fields[i]?fields[i]:"NULL")<<"\n";
	std::cerr<<std::endl;
	return 0;
}
...
int main() {
	...
	int rc=sqlite3_open("rc.sqlite", &db);
	if(rc) std::cerr<<"Can't open database: "<<sqlite3_errmsg(db)<<"\n";
	else std::cerr<<"Open database successfully\n\n";
	...
	//QL[6]="delete from myTable";
	//QL[7]="drop table myTable";
