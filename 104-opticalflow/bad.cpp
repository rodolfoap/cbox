...
int main() {
	...
	cv::Mat frame, frame_old, frame_new;
	cap>>frame;
	resize(frame, frame_old, cv::Size(WIDTH, HEIGHT), cv::INTER_LINEAR);
	...
	while(frame_new.data) {
		...
		cap>>frame;
		resize(frame, frame_new, cv::Size(WIDTH, HEIGHT), cv::INTER_LINEAR);
	}
	cap.release();
	return 0;
}
